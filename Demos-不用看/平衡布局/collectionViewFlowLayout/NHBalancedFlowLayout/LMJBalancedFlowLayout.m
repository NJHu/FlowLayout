...
#import "LMJLinearPartition.h"

@interface LMJBalancedFlowLayout ()
{
...
@implementation LMJBalancedFlowLayout
...
- (void)prepareLayout
{
    ...
    NSAssert([self.delegate conformsToProtocol:@protocol(LMJBalancedFlowLayoutDelegate)], @"UICollectionView delegate should conform to BalancedFlowLayout protocol");
...
- (CGFloat)totalItemSizeForSection:(NSInteger)section preferredRowSize:(CGFloat)preferredRowSize
{
    ...
    for (NSInteger i = 0, n = [self.collectionView numberOfItemsInSection:section]; i < n; i++) {
        CGSize preferredSize = [self.delegate waterflowLayout:self collectionView:self.collectionView sizeForItemAtIndexPath:[NSIndexPath indexPathForItem:i inSection:section]];
...
- (void)setFrames:(CGRect *)frames forItemsInSection:(NSInteger)section numberOfRows:(NSUInteger)numberOfRows sectionOffset:(CGPoint)sectionOffset sectionSize:(CGSize *)sectionSize
{
    ...
    NSArray *partition = [LMJLinearPartition linearPartitionForSequence:weights numberOfPartitions:numberOfRows];
    ...
    for (NSArray *row in partition) {
        ...
        for (NSInteger j = i, n = i + [row count]; j < n; j++) {
            CGSize preferredSize = [self.delegate waterflowLayout:self collectionView:self.collectionView sizeForItemAtIndexPath:[NSIndexPath indexPathForItem:j inSection:section]];
...
- (id<LMJBalancedFlowLayoutDelegate>)delegate
{
    return (id<LMJBalancedFlowLayoutDelegate>)self.collectionView.delegate;
}
...
- (CGSize)referenceSizeForHeaderInSection:(NSInteger)section
{
    ...
    if (respondsToSelector) {
        return [(id <LMJBalancedFlowLayoutDelegate>)self.collectionView.delegate collectionView:self.collectionView layout:self referenceSizeForHeaderInSection:section];
    }
...
- (CGSize)referenceSizeForFooterInSection:(NSInteger)section
{
    ...
    if (respondsToSelector) {
        return [(id <LMJBalancedFlowLayoutDelegate>)self.collectionView.delegate collectionView:self.collectionView layout:self referenceSizeForFooterInSection:section];
    }
