...
#define LMJ_LP_TABLE_LOOKUP(table, i, j, rowsize) (table)[(i) * (rowsize) + (j)]


@implementation LMJLinearPartition
...
+ (NSInteger *)linearPartitionTable:(NSArray *)sequence numPartitions:(NSInteger)numPartitions
{
    ...
    for (NSInteger i = 0; i < n; i++) {
        NSInteger offset = i? LMJ_LP_TABLE_LOOKUP(tmpTable, i - 1, 0, k) : 0;
        LMJ_LP_TABLE_LOOKUP(tmpTable, i, 0, k) = [sequence[i] integerValue] + offset;
    }
    
    for (NSInteger j = 0; j < k; j++) {
        LMJ_LP_TABLE_LOOKUP(tmpTable, 0, j, k) = [sequence[0] integerValue];
    }
    ...
    for (NSInteger i = 1; i < n; i++) {
        for (NSInteger j = 1; j < k; j++) {
            ...
            for (NSInteger x = 0; x < i; x++) {
                NSInteger c1 = LMJ_LP_TABLE_LOOKUP(tmpTable, x, j - 1, k);
                NSInteger c2 = LMJ_LP_TABLE_LOOKUP(tmpTable, i, 0, k) - LMJ_LP_TABLE_LOOKUP(tmpTable, x, 0, k);
            ...
            LMJ_LP_TABLE_LOOKUP(tmpTable, i, j, k) = currentMin;
            LMJ_LP_TABLE_LOOKUP(solution, i - 1, j - 1, k - 1) = minX;
        }
    }
...
+ (NSArray *)linearPartitionForSequence:(NSArray *)sequence numberOfPartitions:(NSInteger)numberOfPartitions
{
    ...
    while (k >= 0) {
        if (n < 1) {
            ...
            for (NSInteger i = LMJ_LP_TABLE_LOOKUP(solution, n - 1, k, solutionRowSize) + 1, range = n + 1; i < range; i++) {
                [currentAnswer addObject:sequence[i]];
            }
            [answer insertObject:currentAnswer atIndex:0];
            
            n = LMJ_LP_TABLE_LOOKUP(solution, n - 1, k, solutionRowSize);
        }
